...
resource "aws_vpc" "main" {
  ...
  tags = {
    Name = "${var.vpc_name}-vpc-jenkins"      # Додаємо тег, який включає ім'я VPC
  }
}
...
resource "aws_subnet" "public" {
  ...
  tags = {
    Name = "${var.vpc_name}-public-subnet-jenkins-${count.index + 1}"  # Тег з нумерацією підмережі
...
resource "aws_subnet" "private" {
  ...
  tags = {
    Name = "${var.vpc_name}-private-subnet-jenkins-${count.index + 1}"  # Тег для підмережі з нумерацією
...
resource "aws_internet_gateway" "igw" {
  ...
  tags = {
    Name = "${var.vpc_name}-igw-jenkins"   # Тег для ідентифікації Internet Gateway
  }
}
...
resource "aws_eip" "nat" {
  ...
  tags = {
    Name = "${var.vpc_name}-nat-eip-jenkins"   # Тег для ідентифікації Elastic IP для NAT Gateway
  }
...
resource "aws_nat_gateway" "nat" {
  ...
  tags = {
    Name = "${var.vpc_name}-nat-gateway-jenkins"   # Тег для ідентифікації NAT Gateway
  }
...
resource "aws_route_table" "private" {
  ...
  tags =  {
    Name = "${var.vpc_name}-private-jenkins-rt"
  }  
}
